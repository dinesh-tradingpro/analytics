...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-10">
        <div>
            <h2 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">Manager Analytics</h2>
            <p class="text-gray-600 dark:text-gray-300">Track manager engagement and response activity</p>
        </div>
    </div>
    ...
    <div class="space-y-8">
                    ...
                    <div class="p-4 bg-white/20 rounded-xl backdrop-blur-sm">
                        <svg class="w-8 h-8 text-white" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            <path fill-rule="evenodd"
                                d="M4 5a2 2 0 012-2 1 1 0 000 2H4a1 1 0 00-1 1v10a1 1 0 001 1h12a1 1 0 001-1V5a1 1 0 00-1-1h-1a1 1 0 000-2 2 2 0 00-2 2v.001h-6V5z"
                                clip-rule="evenodd" />
                        </svg>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div
                class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden border border-gray-200 dark:border-gray-700">
                    ...
                    @if ($this->responsesLast7Days && count($this->responsesLast7Days) > 0)
                        <div class="space-y-3">
                            @foreach (array_slice($this->responsesLast7Days, 0, 8) as $item)
                                <div
                                    class="flex items-center justify-between group hover:bg-blue-50 dark:hover:bg-gray-700/50 px-3 py-2 rounded-lg transition-colors">
                                    <span
                                        class="text-sm font-medium text-gray-700 dark:text-gray-300 flex-1 truncate">{{ $item['name'] }}</span>
                                    <span
                                        class="ml-2 inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-300 whitespace-nowrap">
                                        {{ $item['responses'] }}
                                    </span>
                                </div>
                            @endforeach
                    ...
                    @if ($this->responsesLast30Days && count($this->responsesLast30Days) > 0)
                        <div class="space-y-3">
                            @foreach (array_slice($this->responsesLast30Days, 0, 8) as $item)
                                <div
                                    class="flex items-center justify-between group hover:bg-emerald-50 dark:hover:bg-gray-700/50 px-3 py-2 rounded-lg transition-colors">
                                    <span
                                        class="text-sm font-medium text-gray-700 dark:text-gray-300 flex-1 truncate">{{ $item['name'] }}</span>
                                    <span
                                        class="ml-2 inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-emerald-100 dark:bg-emerald-900/50 text-emerald-800 dark:text-emerald-300 whitespace-nowrap">
                                        {{ $item['responses'] }}
                                    </span>
                                </div>
                            @endforeach
                    ...
                    @if ($this->responsesLastYear && count($this->responsesLastYear) > 0)
                        <div class="space-y-3">
                            @foreach (array_slice($this->responsesLastYear, 0, 8) as $item)
                                <div
                                    class="flex items-center justify-between group hover:bg-pink-50 dark:hover:bg-gray-700/50 px-3 py-2 rounded-lg transition-colors">
                                    <span
                                        class="text-sm font-medium text-gray-700 dark:text-gray-300 flex-1 truncate">{{ $item['name'] }}</span>
                                    <span
                                        class="ml-2 inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-purple-100 dark:bg-purple-900/50 text-purple-800 dark:text-purple-300 whitespace-nowrap">
                                        {{ $item['responses'] }}
                                    </span>
                                </div>
                            @endforeach
